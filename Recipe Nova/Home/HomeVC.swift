...
class HomeVC: UIViewController,HomeVCOperation,UNUserNotificationCenterDelegate {
    ...
    override func viewDidLoad() {
        ...
        UI.CT.sort() ///sort the categories
        ...
        ///Widget was called
        NotificationCenter.default.addObserver(self, selector: #selector(WidgetWasCalled), name: NSNotification.Name(rawValue: "WidgetAction"), object: nil)
    ...
    //MARK: - Handling widget called
    @objc private func WidgetWasCalled(){
            if let userDefaults = UserDefaults(suiteName:"group.iNfamousPersian.Recipe") {
                //Open List
                let openListFromWidget = userDefaults.bool(forKey: "OpenList")
                if openListFromWidget == true{
                    if let userDefaults = UserDefaults(suiteName: "group.iNfamousPersian.Recipe"){
                        userDefaults.set(false, forKey: "OpenList")
                        userDefaults.synchronize()
                    }
                    ///list ID
                    guard let id = userDefaults.string(forKey: "ID") else {
                        return
                    }
                    ///load list from coredata and look for the list
                    SaveLoadListHandler(list: ShoppingCart()).FetchFromCoreData { (list) in
                        list.root?.traversePreOrder(visit: { (cart) in
                        if cart.ID == UUID.init(uuidString: id){
                            let storyboard = UIStoryboard(name: "Main", bundle: Bundle.main)
                            // Instantiate View Controller
                            let viewController = storyboard.instantiateViewController(withIdentifier: "ListTV") as! ListTV
                                    
                            viewController.modalPresentationStyle = .automatic
                            viewController.List = cart
